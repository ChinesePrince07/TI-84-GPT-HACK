...
void setup() {
  ...
  WiFi.begin("Suffield Gear", "REDACTED_WIFI_PASS");
...
const char* CH15_BODIES[] = {
  ...
  // 12: RECT TO CYL
  "RECTANGULAR (x,y,z)>TO CYLINDRICAL (r,@,z)>r=SQRT(x^2+y^2)>@=ARCTAN(y/x)>z=z (UNCHANGED)>WATCH @ QUADRANT>BASED ON SIGNS x,y>r>=0 ALWAYS>0<=@<2*PI>VERIFY:>x=r*COS(@)>y=r*SIN(@)",
  // 13: CYL TO RECT
  "CYLINDRICAL (r,@,z)>TO RECTANGULAR (x,y,z)>x=r*COS(@)>y=r*SIN(@)>z=z (UNCHANGED)>EVALUATE BOTH>SIN AND COS>VERIFY:>r=SQRT(x^2+y^2)>@=ARCTAN(y/x)",
  // 14: CYL TO SPH
  "CYLINDRICAL (r,@,z)>TO SPHERICAL (RHO,@,PHI)>RHO=SQRT(r^2+z^2)>@=@ (UNCHANGED)>PHI=ARCTAN(r/z)>or PHI=ARCCOS(z/RHO)>PHI=ANGLE FROM +Z AXIS>0<=PHI<=PI>RHO>=0>VERIFY:>r=RHO*SIN(PHI)>z=RHO*COS(PHI)",
  // 15: SPH TO CYL
  "SPHERICAL (RHO,@,PHI)>TO CYLINDRICAL (r,@,z)>r=RHO*SIN(PHI)>@=@ (UNCHANGED)>z=RHO*COS(PHI)>PHI=ANGLE FROM +Z AXIS>SIN(PHI)>=0 FOR>0<=PHI<=PI>VERIFY:>RHO=SQRT(r^2+z^2)>PHI=ARCTAN(r/z)",
  // 16: RECT TO SPH
  "RECTANGULAR (x,y,z)>TO SPHERICAL (RHO,@,PHI)>RHO=SQRT(x^2+y^2+z^2)>@=ARCTAN(y/x)>PHI=ARCCOS(z/RHO)>=ARCCOS(z/SQRT(x2+y2+z2))>RHO>=0>0<=PHI<=PI>WATCH @ QUADRANT>VERIFY:>x=RHO*SIN(PHI)*COS(@)>y=RHO*SIN(PHI)*SIN(@)>z=RHO*COS(PHI)",
  // 17: SPH TO RECT
  "SPHERICAL (RHO,@,PHI)>TO RECTANGULAR (x,y,z)>x=RHO*SIN(PHI)*COS(@)>y=RHO*SIN(PHI)*SIN(@)>z=RHO*COS(PHI)>PRODUCT OF 3 TERMS>EVALUATE INSIDE-OUT>SIN(PHI)=RADIAL FACTOR>VERIFY:>RHO=SQRT(x2+y2+z2)>PHI=ARCCOS(z/RHO)>@=ARCTAN(y/x)",
};
...
void eduroam_connect() {
  const char* user = "REDACTED_EAP_USER";
  const char* pass = "REDACTED_EAP_PASS";
...
void suffield_connect() {
  ...
  const char* pass = "REDACTED_WIFI_PASS";
